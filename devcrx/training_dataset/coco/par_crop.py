...
def remove_empty_folders(data_dir):
    dirs = os.listdir(data_dir)
    for dd in dirs:
        fdd = join(data_dir, dd)
        files = os.listdir(fdd)
        if []==files:
            shutil.rmtree(fdd)

def main(dataDir, dataTypes, instanc_size=511, num_threads=8):
    crop_path = join(dataDir, 'crop{:d}'.format(instanc_size))
    ...
    for dataType in dataTypes:
        ...
        # remove empty subfolders in 'set_crop_base_path'
        remove_empty_folders(set_crop_base_path)
...
if __name__ == '__main__':
    ...
    #------------------------------------------------
    since = time.time()
    main(dataDir, dataTypes, instanc_size=511, num_threads=4)
